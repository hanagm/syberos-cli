...
export default class Build {
  ...
  private async pdkPath() {
    ...
    const shell = await shelljs.exec(`cd ${home}/Syberos-Pdk`)
    console.log('stdout---', shell)
    if (stdout) {
      return stdout
    }
  ...
  private async locateKchroot() {
    const { stdout } = await shelljs.exec('locate script/kchroot')

    if (!stdout) {
      throw new Error('未找到SyberOS PDK,请检查开发环境')
    }
