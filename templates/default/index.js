...
const {TARGET_NAMES,TARGET_SIMULATOR_NAMES}=require('../../dist/util/constants');
...
exports.createApp = function (creater, params, helper, cb) {
  const { projectName, projectDir, description, template, typescript, date, src, css , sopid } = params
  // syberos app 模块目录
  ...
  // 拷贝APP模板工程
  fs.copySync(path.join(creater.templatePath(),template,platformsDirName),platformsDir);

  // 拷贝Lib模板工程
  fs.copySync(path.join(creater.templatePath(),template,libDir),path.join(sourceDir, 'lib'));
  ...
  creater.fs.commit(() => {
    ...
    console.log(`${chalk.green('✔ ')}${chalk.grey(`创建模板目录: ${projectName}/${platformsDirName}`)}`)
    console.log(`${chalk.green('✔ ')}${chalk.grey(`创建www目录: ${projectName}/${src}`)}`)

    console.log(`${chalk.green('✔ ')}${chalk.grey(`拷贝APP模板: ${projectName}/${src}/${platformsDirName}/${syberosDir}`)}`)

    if (typescript) {
      console.log(`${chalk.green('✔ ')}${chalk.grey(`创建页面 JS 文件: ${projectName}/${src}/index.tsx`)}`)
    } else {
      console.log(`${chalk.green('✔ ')}${chalk.grey(`创建页面 JS 文件: ${projectName}/${src}/index.js`)}`)
    }
    console.log(`${chalk.green('✔ ')}${chalk.grey(`创建页面 ${currentStyleExt.toLocaleUpperCase()} 文件: ${projectName}/${src}/${pageCSSName}`)}`)
    if (typescript) {
      console.log(`${chalk.green('✔ ')}${chalk.grey(`创建文件: ${projectName}/${src}/index.tsx`)}`)
    } else {
    ...
    console.log(`${chalk.green('✔ ')}${chalk.grey(`创建文件: ${projectName}/${platformsDirName}/index.js`)}`)
    //console.log(`${chalk.green('✔ ')}${chalk.grey(`创建文件: ${projectName}/${platformsDirName}/dev.js`)}`)
    //console.log(`${chalk.green('✔ ')}${chalk.grey(`创建文件: ${projectName}/${platformsDirName}/prod.js`)}`)
